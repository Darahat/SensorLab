...
class _MyAppState extends State<MyApp> {
  ...
  Future<void> initPlatformState() async {
    ...
    tempAvailable = await environmentSensors.getSensorAvailable(
      SensorType.AmbientTemperature,
    );
    humidityAvailable = await environmentSensors.getSensorAvailable(
      SensorType.Humidity,
    );
    lightAvailable = await environmentSensors.getSensorAvailable(
      SensorType.Light,
    );
    pressureAvailable = await environmentSensors.getSensorAvailable(
      SensorType.Pressure,
    );
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
        appBar: AppBar(title: const Text('Environment Sensors')),
        body: Column(
